...
<!-- Compacto a propósito: agregar/quitar reglas vive en Inventario → Recargo (solo admin);
...
{#if reglas.length > 0}
	<div class="flex flex-col gap-2 rounded-xl border-2 border-stone-200 bg-white p-3">
		{#each reglas as regla (regla.id)}
			<div class="flex items-center justify-between gap-3 px-1 py-0.5">
				<div class="flex min-w-0 items-center gap-2">
					<Moon size={14} class="shrink-0 {regla.activo ? 'text-yellow-500' : 'text-stone-400'}" />
